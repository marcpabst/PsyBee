...
impl<S: BaseStimulusImplementation> Renderable for BaseStimulus<S> {
    async fn prepare(
        ...
        let geometry =
            geometry.unwrap_or_else(|| simp.get_geometry());
    ...
    fn render(
        ...
        let texture_bind_group = if let Some(texture_bind_group) =
...
impl<S: BaseStimulusImplementation> BaseStimulus<S> {
    ...
    pub fn create(
        ...
        let fragment_shader_code =
            implementation.get_fragment_shader_code();
        ...
        let fragment_shader = device.create_shader_module(
            wgpu::ShaderModuleDescriptor {
                ...
                source: wgpu::ShaderSource::Wgsl(
                    (include_str!("shaders/base.wgsl").to_string()
        ...
        let vertex_shader = device.create_shader_module(
            wgpu::ShaderModuleDescriptor {
                label: None,
                source: wgpu::ShaderSource::Wgsl(
                    include_str!("shaders/base.wgsl").into(),
                ),
            },
        );
        ...
        let vertices = shape.to_vertices_px(
            width_mm,
            viewing_distance_mm,
            width_px,
            height_px,
        ...
        let stimulus_buffer = device.create_buffer_init(
            &wgpu::util::BufferInitDescriptor {
        ...
        let oot = Self {
            ...
            window: window.clone(),
            stimulus_implementation: Arc::new(Mutex::new(
                implementation,
            )),
            transforms: Arc::new(Mutex::new(
