...
impl TextureSize {
    /// Returns a tuple with the type of texture size and the size.
    /// The type is:
    /// - 0 for `TextureSize::Original`
    /// - 1 for `TextureSize::Absolute`
    /// - 2 for `TextureSize::Relative`
...
impl Material {
    ...
    pub fn fragment_shader_module(&self, device: &wgpu::Device) -> wgpu::ShaderModule {
        match self {
            Self::Colour(..) => device.create_shader_module(wgpu::include_wgsl!("shaders/colour.wgsl")),
            Self::Texture(..) => device.create_shader_module(wgpu::include_wgsl!("shaders/texture.wgsl")),
            Self::Gradient(..) => device.create_shader_module(wgpu::include_wgsl!("shaders/gradient.wgsl")),
        }
    }
    ...
    pub fn uniform_bytes(&self) -> Vec<u8> {
        match self {
            ...
            Self::Gradient(GradientMaterial { centre, rotation, .. }) => {
    ...
    pub fn texture_repeat_modes(&self) -> Option<(TextureRepeat, TextureRepeat)> {
        match self {
            Self::Texture(TextureMaterial { repeat_x, repeat_y, .. }) => Some((*repeat_x, *repeat_y)),
            _ => None,
        }
    }
