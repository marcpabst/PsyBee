...
use geometry::PixelFilter;
pub use geometry::Primitive;
pub use geometry::Transformation;
use gst::debug;
...
pub use material::Colour;

pub use geometry::TessellationOptions;
...
use uniform_structs::PixelFillterUniforms;
...
const VERRTEX_BUFFER_SIZE_MB: u32 = 500;
const INDEX_BUFFER_SIZE_MB: u32 = 500;
const UNIFORM_BUFFER_SIZE_MB: u32 = 500;
...
pub struct Renderer {
    ...
    /// The global pixel filter buffer.
    pixel_filter_buffer: wgpu::Buffer,
...
pub struct RenderData {
    ...
    pub pixel_filter_buffer_offsets: Vec<u32>,
...
impl Renderable {
    pub fn as_geom(self) -> Option<Geom> {
        match self {
            Renderable::Geom(geom) => Some(geom),
            _ => None,
        }
    }
}
...
impl Renderer {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        // !TODO: This should dynamically grow as needed.
        ...
        // Create the global uniform buffer that will store all pixel filter data for all the primitives.
        // Per renderable object, this will contain one u32 value (the filter type) and 10 f32 values (the filter parameters).
        let pixel_filter_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Pixel Filter Buffer"),
            size: (UNIFORM_BUFFER_SIZE_MB * 1024) as u64,
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        ...
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            entries: &[
                ...
                // buffer 2 contains pixel filter data
                wgpu::BindGroupLayoutEntry {
                    binding: 2,
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                // again, buffer 0 contains uniform data
                wgpu::BindGroupEntry {
                    binding: 0,
    ...
    pub fn get_texture_bind_group(
        ...
        let texture_bind_group_layout = material_instance
            ...
            .as_ref()
            .expect("Material instance does not have a texture bind group layout. This should not happen.");
    ...
    pub fn prepare(
        ...
        let mut pixel_filter_uniform_offsets: Vec<u32> = vec![];
        ...
        {
            let mut uniforms_staging_buffer = queue
                .write_buffer_with(
                    &self.uniform_buffer,
                    0,
            ...
            // figure out the size of each uniform for the pixel filter
            let pixel_filter_uniforms_size = std::cmp::max(
                PixelFilter::uniform_len(),
                offset_alignment * ((PixelFilter::uniform_len() + offset_alignment - 1) / offset_alignment),
            );

            let mut pixel_filter_staging_buffer = queue
                .write_buffer_with(
                    &self.pixel_filter_buffer,
                    0,
                    NonZeroU64::new((pixel_filter_uniforms_size * geoms.len()) as u64 + offset_alignment as u64)
                        .unwrap(),
                )
                .expect("Failed to write buffer");
            ...
            uniforms_staging_buffer[0..std::mem::size_of::<ScreenUniforms>()]
                .copy_from_slice(bytemuck::bytes_of(&screen_uniforms));

            // next, we prepare the uniform buffer for the primitives
            // there is a lot of potential for optimisation here - but for now we just
            // copy the data into the buffer for every frame
            for (i, geom) in geoms.iter().enumerate() {
                // DEAL WITH THE DRAW BUFFER //
                draw_buffer_collector.tesselate(&geom.primitive, &geom.options);

                // DEAL WITH THE UNIFORM BUFFER //
                ...
                let current_uniform_length =
                    (material_uniforms.len() + primitive_uniforms_len + offset_alignment - 1) & !(offset_alignment - 1);

                // copy the uniforms into the buffer at the correct offset
                uniforms_staging_buffer[(current_uniform_offset as usize)
                ...
                uniforms_staging_buffer[(current_uniform_offset as usize + primitive_uniforms_len)
                    ..(current_uniform_offset as usize + material_uniforms.len() + primitive_uniforms_len as usize)]
                ...
                // DEAL WITH THE PIXEL FILTER BUFFER //
                let pixel_filter = geom.pixel_filter.unwrap_or(PixelFilter::None);
                let pixel_filter_uniforms = pixel_filter.uniforms();

                let current_pixel_filter_offset = pixel_filter_uniforms_size * i;

                // copy the uniforms into the buffer at the correct offset
                pixel_filter_staging_buffer[(current_pixel_filter_offset as usize)
                    ..(current_pixel_filter_offset as usize + PixelFillterUniforms::len())]
                    .copy_from_slice(bytemuck::bytes_of(&pixel_filter_uniforms));

                pixel_filter_uniform_offsets.push(current_pixel_filter_offset as u32);
            }
        }

        // // write the pixel filter buffer
        // let mut empty_array = vec![0; offset_alignment];
        // // write the u32 1 to the buffer (for testing purposes)
        // empty_array[0..4].copy_from_slice(&0u32.to_ne_bytes());
        // queue.write_buffer(&self.pixel_filter_buffer, 0, &empty_array);
        ...
        for geom in geoms {
            ...
            if let Some(texture) = geom.material.texture() {
                ...
                texture_bind_groups.push(Some(self.get_texture_bind_group(&device, &texture, &material)));
            } else {
        ...
        RenderData {
            ...
            pixel_filter_buffer_offsets: pixel_filter_uniform_offsets,
            texture_bind_groups,
        }
    }
    ...
    pub fn render<'rpass>(
        ...
        for (i, primitive) in primitives.iter().enumerate() {
            ...
            let pixel_filter_offset = rdata.pixel_filter_buffer_offsets[i];

            rpass.set_bind_group(0, &self.bind_group, &[0, uniform_offset, pixel_filter_offset]);
