...
impl ImageStimulus {
    pub fn from_image(image: renderer::image::DynamicImage, params: ImageParams) -> Self {
        Self {
            id: Uuid::new_v4(),
            transformation: crate::visual::geometry::Transformation2D::Identity(),
            animations: Vec::new(),
            visible: true,
            image: renderer::VelloImage::new(image),
            params,
        }
    }
...
impl PyImageStimulus {
    ...
    fn __new__(
        ...
        (
            Self(),
            PyStimulus(Arc::new(std::sync::Mutex::new(ImageStimulus::from_path(
                src,
                ImageParams {
                    x: x.into(),
                    y: y.into(),
