    ...
    ($wrapper:ident, $name:ident) => {
        ...
        impl $wrapper {
            ...
            fn __setitem__(mut slf: PyRefMut<'_, Self>, name: &str, value: Py<PyAny>) -> PyResult<()> {
                ...
                // convert the value to a StimulusParam
                let value = match value.extract::<IntoSize>(py) {
                    Ok(val) => StimulusParamValue::Size(val.into()),
                    Err(_) => match value.extract::<f64>(py) {
                        Ok(val) => StimulusParamValue::f64(val),
                        Err(_) => return Err(PyValueError::new_err("invalid value type")),
                    },
                };

                downcast_stimulus_mut!(slf, $name).set_param(name, value);
                Ok(())
            }
...
impl WrappedImage {
    ...
    pub fn inner(&self) -> renderer::VelloImage {
